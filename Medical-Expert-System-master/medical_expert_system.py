...
class Greetings(KnowledgeEngine):
	...
	def _initial_action(self):
		...
		print("Para ello tendrás que responder a algunas preguntas sobre tus condiciones")
		print("¿Siente alguno de los síntomas siguientes?:")
		print("")
	...
	def symptom_6(self):
		self.declare(Fact(sunken_eyes=input("los ojos hundidos: ")))
	
	@Rule(Fact(action='find_disease'), NOT(Fact(low_body_temp=W())),salience = 1)
	def symptom_7(self):
		self.declare(Fact(low_body_temp=input("temperatura corporal baja: ")))
	
	@Rule(Fact(action='find_disease'), NOT(Fact(restlessness=W())),salience = 1)
	def symptom_8(self):
		self.declare(Fact(restlessness=input("intranquilidad: ")))
	...
	def symptom_11(self):
		self.declare(Fact(nausea=input("Náuseas: ")))

	@Rule(Fact(action='find_disease'), NOT(Fact(blurred_vision=W())),salience = 1)
	def symptom_12(self):
		self.declare(Fact(blurred_vision=input("visión borrosa: ")))
